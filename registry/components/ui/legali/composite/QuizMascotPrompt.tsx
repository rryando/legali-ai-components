...
const QuizMascotPrompt = React.forwardRef<HTMLDivElement, QuizMascotPromptProps>(
  (
    ...
    const scriptLoopTimerRef = React.useRef<number | null>(null)

    const clearScriptLoopTimer = React.useCallback(() => {
      if (scriptLoopTimerRef.current != null) {
        window.clearTimeout(scriptLoopTimerRef.current)
        scriptLoopTimerRef.current = null
      }
    }, [])
    ...
    const streamConfig = React.useMemo(() => {
      return {
        ...
        linePauseMs: stream?.linePauseMs ?? 3000,
    ...
    const effectiveLinePauseMs = React.useMemo(() => {
      if (!streamConfig.fitToStepDuration || step.durationMs == null) {
        return streamConfig.linePauseMs
      }

      const pausesCount = Math.max(1, stepLines.length - 1)
      const maxPausePerBreak = Math.floor((step.durationMs * 0.35) / pausesCount)
      const minPause = 400

      return Math.max(minPause, Math.min(streamConfig.linePauseMs, maxPausePerBreak))
    }, [step.durationMs, stepLines.length, streamConfig.fitToStepDuration, streamConfig.linePauseMs])
    ...
    const computeTypingSpeedMs = React.useCallback(() => {
      ...
      const pauses = Math.max(0, stepLines.length - 1) * effectiveLinePauseMs
      ...
      step.durationMs,
      stepLines,
      effectiveLinePauseMs,
      streamConfig.charIntervalMs,
      streamConfig.fitToStepDuration,
    ])

    const isWaitingAnswer = React.useMemo(() => {
      if (typeof triggerKey !== "string") return false
      return triggerKey.includes("onWaitingAnswer") || triggerKey.includes(":hint:")
    }, [triggerKey])

    React.useEffect(() => {
      clearScriptLoopTimer()

      if (!active || !isWaitingAnswer || !streamConfig.loop) return
      if (!activeScript.length) return

      const isLastStep = stepIndex >= activeScript.length - 1
      if (!isLastStep) return

      const totalChars = stepLines.reduce((sum, l) => sum + l.length, 0)
      const typingDuration = totalChars * computeTypingSpeedMs()
      const pauseDuration = Math.max(1, stepLines.length) * effectiveLinePauseMs
      const restartDelay = Math.max(1200, (step.durationMs ?? 0) + typingDuration + pauseDuration)

      scriptLoopTimerRef.current = window.setTimeout(() => {
        startScript(activeScript)
      }, restartDelay)

      return () => clearScriptLoopTimer()
    }, [
      active,
      activeScript,
      clearScriptLoopTimer,
      computeTypingSpeedMs,
      effectiveLinePauseMs,
      isWaitingAnswer,
      startScript,
      step.durationMs,
                ...
                {prefersReducedMotion ? (
                  ...
                  active ? (
                    <TypingText
                      ...
                      pauseDuration={effectiveLinePauseMs}
                      startOnView={false}
                      once={false}
                    />
                  ) : null
