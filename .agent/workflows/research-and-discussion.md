...
Non-Negotiables
1. Begin every engagement by reviewing existing knowledge under `.agent/knowledge` and the active task's `meta.json`.
...
2. Confirm the task slug from `TASK_SLUG`; if not provided, prompt for it. Ensure a directory exists at `.agent/tasks/<task-slug>/research`, creating it programmatically if missing.
3. If this is a new task, copy the meta template (`.codex/templates/meta.template.json` when present, otherwise `.agent/workflows/templates/meta.template.json`) to `.agent/tasks/<task-slug>/meta.json`; populate `task_id`, `task_slug`, `title`, and initial `summary` plus `created_at`/`updated_at`, using the provided `TITLE`, `REQUEST`, and requester info (`AUTHOR`, `SOURCE`) when supplied.
4. Maintain `.agent/tasks/<task-slug>/meta.json`:
...
5. Capture the deliverable using the research template (`.agent/templates/research_output.md` or, if absent locally,) and save it to `.agent/tasks/<task-slug>/research/<task-slug>-research.md`.
6. Highlight any recommended next checks for Planning inside the brief and in `meta.json.open_questions`.

Deliverable
- File: `.agent/tasks/<task-slug>/research/<task-slug>-research.md`
- Format: `.agent/templates/research_output.md`
